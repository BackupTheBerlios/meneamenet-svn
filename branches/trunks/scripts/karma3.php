...
/////
		...
		$published_given = $db->get_var("SELECT SQL_NO_CACHE count(*) FROM votes,links WHERE vote_type='links' and vote_user_id = $user->id and vote_date > FROM_UNIXTIME($history_from)  and vote_value > 0 AND link_id = vote_link_id AND link_status = 'published' AND vote_date < link_published_date");
	...
	if ($user->karma == $karma) {
		...
		if ($user->karma > $max_karma * 0.75 && $user->level == 'normal') {
			...
			if ($user->level == 'special' && $user->karma < $max_karma * 0.65) {
				$user->level = 'normal';
			}
		}
		$user->store();
