...
if(($from = check_integer('range')) >= 0 && $from < count($range_values) && $range_values[$from] > 0 ) {
	...
	$from_where = "FROM votes, links WHERE  vote_type='links' and vote_date > FROM_UNIXTIME($from_time) AND vote_link_id=link_id AND";
	$time_link = "link_modified > FROM_UNIXTIME($from_time) AND";
} else {
	$from_where = "FROM votes, links WHERE vote_type='links' and vote_link_id=link_id AND vote_value > 0 AND";
	$time_link = '';
}
