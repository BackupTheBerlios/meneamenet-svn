...
{
	url = 'backend/checkfield.php?type='+type+'&name=' + field.value;
...
function report_problem(frm, user, id, md5 /*id, code*/) {
	...
	url="backend/problem.php?" + content;
...
function get_votes(program,type,container,page,id) {
	var url = 'backend/'+program+'?id='+id+'&p='+page+'&type='+type;
	xmlhttp.open('get', url, true);
	xmlhttp.onreadystatechange = function () {
		if(xmlhttp.readyState == 4){
			response = xmlhttp.responseText;
