...
function check_chat() {
	...
	if ($current_user->user_id > 0 && strlen($comment) > 2) {
		if (preg_match('/^!/', $comment)) {
			...
			// Text effects
			/* moved to later, to save space in DB
			$comment = text_to_html(htmlspecialchars($comment));
			$comment = preg_replace('/(^|[\s\.,¿])\/me([\s\.,\?]|$)/', "$1<i>$current_user->user_login</i>$2", $comment);
			$comment = preg_replace('/(^|[\s\.,¿])_([^\s]+)_([\s\.,\?]|$)/', "$1<i>$2</i>$3", $comment);
			$comment = preg_replace('/(^|[\s\.,¿])\*([^\s]+)\*([\s\.,\?]|$)/', "$1<b>$2</b>$3", $comment);
			*/
